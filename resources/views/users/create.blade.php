<x-app-layout title="Tambah Pengguna">
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
            ...
            <x-card title="Data Akun & Departemen Organisasi" subtitle="Identitas pengguna, alamat email masuk, dan klasifikasi divisi" variant="primary">
                            ...
                            <x-select name="divisi_id" placeholder="-- Pilih Divisi / Departemen --">
                                @if(isset($divisis) && $divisis->isNotEmpty())
                                    @foreach($divisis as $d)
                                        <option value="{{ $d->id }}" @selected((string) old('divisi_id') === (string) $d->id)>{{ $d->nama }} ({{ $d->kode }})</option>
                                    @endforeach
                                @else
                                    @foreach($divisiList as $divKey => $divLabel)
                                        <option value="{{ $divKey }}" @selected(old('divisi') === $divKey)>{{ $divLabel }}</option>
                                    @endforeach
                                @endif
                            </x-select>
                        </x-form-group>
            ...
            <div x-data="{
                accessType: '{{ old('warehouse_access_type', 'global') }}',
                selectedGudangs: {{ json_encode(array_map('intval', (array) old('gudang_ids', []))) }},
                primaryGudang: '{{ old('primary_gudang_id', '') }}',
            ...
            <div x-data="{ openDirectPerms: false }">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between p-3 rounded-sm bg-blue-50/60 border border-blue-200/80 text-blue-800 text-xs">
                            <div class="flex items-center gap-2">
                                ...
                                <span>Secara default, pengguna mewarisi seluruh wewenang dari <strong>Peran Utama (Role)</strong>. Matriks ini hanya digunakan jika Anda ingin memberikan wewenang ekstra.</span>
                            </div>
                            <button type="button" 
                        ...
                        <div x-show="openDirectPerms" x-transition>
                            <x-permission-matrix :catalog="$catalog" :activePermissions="old('permissions', [])" />
                        </div>
                    </div>
