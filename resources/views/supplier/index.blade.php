<x-app-layout title="Supplier">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold text-gray-900">Master Supplier</h3>
        @can('supplier.create')
        <a href="{{ route('supplier.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-primary-500 text-white text-sm font-medium rounded-xl hover:bg-primary-600 transition shadow-sm">
    ...
    <div class="table-wrapper bg-white rounded-xl border border-gray-200 p-4">
        <table id="tbl" class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200"><tr>
                ...
                <th class="px-4 py-3 text-left font-medium text-gray-600">Kontak</th>
    ...
    $(function(){ $('#tbl').DataTable({ processing:true, serverSide:true, ajax:'{{ route("supplier.data") }}',
        columns:[{data:'nama'},{data:'kategori'},{data:'kontak'},{data:'termin_default'},{data:'is_active',name:'is_active'},{data:'action',orderable:false,searchable:false}],
