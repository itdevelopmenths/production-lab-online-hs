    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div x-data="poForm()">
            ...
            <div class="space-y-5">
                ...
                <x-card title="Daftar Bahan Baku & Biaya Tambahan" subtitle="Input kuantitas, total harga, diskon & PPN (bisa pilih % atau Rp), ongkir, dan adjustment. HPP per unit dihitung otomatis." :noPadding="true" class="relative z-20">
                    <div class="overflow-visible">
                        <table class="w-full text-xs">
                            <thead class="bg-gray-50 border-b border-gray-200 text-gray-700 font-semibold uppercase text-[11px] tracking-wider">
                                <tr>
                                    <th class="px-3 py-2.5 text-left w-[24%]">Bahan Baku <span class="text-rose-500">*</span></th>
                                    <th class="px-2 py-2.5 text-right w-36">Qty <span class="text-rose-500">*</span></th>
                                    <th class="px-2 py-2.5 text-right w-28">Total Harga <span class="text-rose-500">*</span></th>
                                    <th class="px-2 py-2.5 text-right w-32">Diskon (% / Rp)</th>
                                    <th class="px-2 py-2.5 text-right w-32">PPN (% / Rp)</th>
                                    <th class="px-2 py-2.5 text-right w-24">Ongkir</th>
                                    <th class="px-2 py-2.5 text-right w-20">Adj</th>
                                    <th class="px-3 py-2.5 text-right w-28 bg-primary-50/50 text-primary-900">HPP / Unit</th>
                                    <th class="px-2 py-2.5 text-center w-10">Aksi</th>
                                </tr>
                            </thead>
                                    ...
                                    >
                                        ...
                                        <td class="px-2 py-2">
                                            <div class="relative flex rounded-sm shadow-2xs">
                                                ...
                                                />
                                                <span class="inline-flex items-center px-1.5 py-1 text-[10px] font-mono font-bold text-gray-600 bg-gray-100 border border-l-0 border-gray-300 rounded-r-sm shrink-0 uppercase select-none min-w-[36px] justify-center" x-text="row.satuan || 'UNIT'"></span>
                                            </div>
                                        </td>
                                        ...
                                        {{-- Kolom 4: Diskon (% atau Rp) --}}
                                        <td class="px-2 py-2">
                                            <input type="hidden" :name="`items[${i}][diskon]`" :value="row.diskon">
                                            <div class="relative flex rounded-sm shadow-2xs">
                                                <button
                                                    type="button"
                                                    @click="toggleRowDiskonMode(row)"
                                                    class="px-1.5 py-1 text-[10px] font-mono font-bold rounded-l-sm border border-r-0 transition cursor-pointer select-none shrink-0"
                                                    :class="row.diskon_mode === 'persen' ? 'bg-rose-100 border-rose-300 text-rose-800 hover:bg-rose-200' : 'bg-gray-100 border-gray-300 text-gray-700 hover:bg-gray-200'"
                                                    :title="'Klik untuk ganti mode: ' + (row.diskon_mode === 'persen' ? 'Persentase (%)' : 'Nominal (Rp)')"
                                                >
                                                    <span x-text="row.diskon_mode === 'persen' ? '%' : 'Rp'"></span>
                                                </button>
                                                <template x-if="row.diskon_mode === 'persen'">
                                                    <input
                                                        type="number"
                                                        step="any"
                                                        min="0"
                                                        max="100"
                                                        x-model.number="row.diskon_val"
                                                        @input="recalcRow(row)"
                                                        @keydown.enter.prevent
                                                        placeholder="0"
                                                        class="w-full rounded-r-sm border-gray-300 text-xs py-1.5 px-1.5 font-mono text-right focus:border-primary-500 focus:ring-1 focus:ring-primary-500 text-rose-600"
                                                    />
                                                </template>
                                                <template x-if="row.diskon_mode !== 'persen'">
                                                    <input
                                                        type="text"
                                                        inputmode="numeric"
                                                        :value="formatThousand(row.diskon)"
                                                        @input="updateRowField(row, 'diskon', $event)"
                                                        @keydown="handleKeyDown($event)"
                                                        placeholder="0"
                                                        class="w-full rounded-r-sm border-gray-300 text-xs py-1.5 px-1.5 font-mono text-right focus:border-primary-500 focus:ring-1 focus:ring-primary-500 text-rose-600"
                                                    />
                                            ...
                                            <div class="text-[9px] text-right font-mono text-gray-400 mt-0.5 truncate" x-show="row.diskon > 0">
                                                <span x-show="row.diskon_mode === 'persen'" x-text="'= Rp ' + formatThousand(row.diskon)"></span>
                                                <span x-show="row.diskon_mode !== 'persen' && row.harga > 0" x-text="'(' + (Math.round((row.diskon / row.harga) * 1000) / 10) + '%)'"></span>
                                            </div>
                                        </td>

                                        {{-- Kolom 5: PPN (% atau Rp) --}}
                                        <td class="px-2 py-2">
                                            <input type="hidden" :name="`items[${i}][ppn]`" :value="row.ppn">
                                            <div class="relative flex rounded-sm shadow-2xs">
                                                <button
                                                    type="button"
                                                    @click="toggleRowPpnMode(row)"
                                                    class="px-1.5 py-1 text-[10px] font-mono font-bold rounded-l-sm border border-r-0 transition cursor-pointer select-none shrink-0"
                                                    :class="row.ppn_mode === 'persen' ? 'bg-primary-100 border-primary-300 text-primary-800 hover:bg-primary-200' : 'bg-gray-100 border-gray-300 text-gray-700 hover:bg-gray-200'"
                                                    :title="'Klik untuk ganti mode: ' + (row.ppn_mode === 'persen' ? 'Persentase (%)' : 'Nominal (Rp)')"
                                                >
                                                    <span x-text="row.ppn_mode === 'persen' ? '%' : 'Rp'"></span>
                                                </button>
                                                <template x-if="row.ppn_mode === 'persen'">
                                                    <input
                                                        type="number"
                                                        step="any"
                                                        min="0"
                                                        max="100"
                                                        x-model.number="row.ppn_val"
                                                        @input="recalcRow(row)"
                                                        @keydown.enter.prevent
                                                        placeholder="0"
                                                        class="w-full rounded-r-sm border-gray-300 text-xs py-1.5 px-1.5 font-mono text-right focus:border-primary-500 focus:ring-1 focus:ring-primary-500 text-gray-900"
                                                    />
                                                </template>
                                                <template x-if="row.ppn_mode !== 'persen'">
                                                    <input
                                                        type="text"
                                                        inputmode="numeric"
                                                        :value="formatThousand(row.ppn)"
                                                        @input="updateRowField(row, 'ppn', $event)"
                                                        @keydown="handleKeyDown($event)"
                                                        placeholder="0"
                                                        class="w-full rounded-r-sm border-gray-300 text-xs py-1.5 px-1.5 font-mono text-right focus:border-primary-500 focus:ring-1 focus:ring-primary-500 text-gray-900"
                                                    />
                                            ...
                                            <div class="text-[9px] text-right font-mono text-gray-400 mt-0.5 truncate" x-show="row.ppn > 0">
                                                <span x-show="row.ppn_mode === 'persen'" x-text="'= Rp ' + formatThousand(row.ppn)"></span>
                                                <span x-show="row.ppn_mode !== 'persen' && (row.harga - row.diskon) > 0" x-text="'(' + (Math.round((row.ppn / Math.max(1, row.harga - row.diskon)) * 1000) / 10) + '%)'"></span>
                                            </div>
                                        </td>

                                        {{-- Kolom 6: Ongkir --}}
                                        <td class="px-2 py-2">
                                            <input type="hidden" :name="`items[${i}][ongkir]`" :value="row.ongkir">
                                            <input
                                                type="text"
                                                inputmode="numeric"
                                                :value="formatThousand(row.ongkir)"
                                                @input="updateRowField(row, 'ongkir', $event)"
                                                @keydown="handleKeyDown($event)"
                                                placeholder="0"
                                                class="w-full rounded-sm border-gray-300 text-xs py-1.5 px-2 font-mono text-right focus:border-primary-500 focus:ring-1 focus:ring-primary-500 shadow-xs"
                                            />
                                        ...
                                        {{-- Kolom 7: Adjustment --}}
                                        <td class="px-2 py-2">
                                            <input type="hidden" :name="`items[${i}][adjustment]`" :value="row.adjustment">
                                            <input
                                                type="text"
                                                inputmode="numeric"
                                                :value="formatThousand(row.adjustment)"
                                                @input="updateRowField(row, 'adjustment', $event)"
                                                @keydown="handleKeyDown($event)"
                                                placeholder="0"
                                                class="w-full rounded-sm border-gray-300 text-xs py-1.5 px-2 font-mono text-right focus:border-primary-500 focus:ring-1 focus:ring-primary-500 shadow-xs"
                                            />
                                        ...
                                        {{-- Kolom 8: Live HPP / Unit --}}
                                        <td class="px-3 py-2 text-right bg-primary-50/40">
                                            <div class="font-mono font-bold text-xs text-primary-800" x-text="'Rp ' + formatThousand(calculateRowHpp(row))"></div>
                                            <div class="text-[10px] text-gray-500 font-mono" x-text="'Net: Rp ' + formatThousand(calculateRowNet(row))"></div>
                                        </td>
                ...
                {{-- Card 4: Jadwal Pembayaran Termin (Hanya muncul jika skema_bayar == termin) --}}
                                ...
                                <tbody class="divide-y divide-gray-100 bg-white">
                                    <template x-for="(tm, idx) in termins" :key="idx">
                                        <tr class="hover:bg-gray-50/50">
                                            ...
                                            <td class="px-4 py-2">
                                                <input type="date" :name="`termins[${idx}][tanggal_tempo]`" x-model="tm.tanggal_tempo" required class="w-full text-xs py-1.5 rounded-sm border-gray-300 focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
                                            </td>
                                            <td class="px-4 py-2">
                                                <input type="hidden" :name="`termins[${idx}][nominal_tagihan]`" :value="tm.nominal_tagihan">
                                                <input type="text" inputmode="numeric" :value="formatThousand(tm.nominal_tagihan)" @input="updateTerminNominal(tm, $event)" @keydown="handleKeyDown($event)" required placeholder="0" class="w-full text-right font-mono text-xs py-1.5 rounded-sm border-gray-300 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 font-semibold text-gray-900">
                                            </td>
                                            <td class="px-4 py-2">
                                                <input type="text" :name="`termins[${idx}][keterangan]`" x-model="tm.keterangan" placeholder="Contoh: DP 30% / Pelunasan" class="w-full text-xs py-1.5 rounded-sm border-gray-300 focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
                                            </td>
    ...
    <script>
        function poForm() {
            return {
                ...
                toggleRowDiskonMode(row) {
                    if (row.diskon_mode === 'persen') {
                        row.diskon_mode = 'nominal';
                    } else {
                        row.diskon_mode = 'persen';
                        if (!row.diskon_val && row.diskon > 0 && row.harga > 0) {
                            row.diskon_val = Math.round(((parseFloat(row.diskon) || 0) / (parseFloat(row.harga) || 1)) * 10000) / 100;
                        }
                    }
                    this.recalcRow(row);
                },

                toggleRowPpnMode(row) {
                    if (row.ppn_mode === 'persen') {
                        row.ppn_mode = 'nominal';
                    } else {
                        row.ppn_mode = 'persen';
                        const dpp = Math.max(0, (parseFloat(row.harga) || 0) - (parseFloat(row.diskon) || 0));
                        if (!row.ppn_val && row.ppn > 0 && dpp > 0) {
                            row.ppn_val = Math.round(((parseFloat(row.ppn) || 0) / dpp) * 10000) / 100;
                        } else if (!row.ppn_val) {
                            row.ppn_val = 11;
                        }
                    }
                ...
                recalcRow(row) {
                    ...
                    if (row.diskon_mode === 'persen') {
                        const pct = parseFloat(row.diskon_val) || 0;
                        row.diskon = Math.round((h * pct) / 100);
                    }
                    
                    if (row.ppn_mode === 'persen') {
                        const dpp = Math.max(0, h - (parseFloat(row.diskon) || 0));
                        const pct = parseFloat(row.ppn_val) || 0;
                        row.ppn = Math.round((dpp * pct) / 100);
                    }
                ...
                recalcAll() {
                    this.rows.forEach(r => this.recalcRow(r));
                ...
                updateHeaderField(field, event) {
                    ...
                    if (rawDigits === '') {
                        this[field] = 0;
                        input.value = '';
                        if (field === 'headerDiskon' && this.headerPpnMode === 'persen') this.recalcHeaderPpn();
                        return;
                    }
                    if (rawDigits.length > 1 && rawDigits.startsWith('0')) {
                        rawDigits = String(parseInt(rawDigits, 10) || 0);
                ...
                calculateRowNet(row) {
                    ...
                    const p = parseFloat(row.ppn) || 0;
                    const o = parseFloat(row.ongkir) || 0;
                    const a = parseFloat(row.adjustment) || 0;
                    return Math.max(0, h - d + p + o + a);
                },

                calculateRowHpp(row) {
                    const q = parseFloat(row.qty) || 0;
                    if (q <= 0) return 0;
                    return Math.round((this.calculateRowNet(row) / q) * 100) / 100;
                },
                ...
                totalDiskon() {
                    const itemDiskon = this.rows.reduce((acc, r) => acc + (parseFloat(r.diskon) || 0), 0);
                    return itemDiskon + (parseFloat(this.headerDiskon) || 0);
                },

                totalPpn() {
                    const itemPpn = this.rows.reduce((acc, r) => acc + (parseFloat(r.ppn) || 0), 0);
                    return itemPpn + (parseFloat(this.headerPpn) || 0);
                },

                totalOngkir() {
                    const itemOngkir = this.rows.reduce((acc, r) => acc + (parseFloat(r.ongkir) || 0), 0);
                    return itemOngkir + (parseFloat(this.headerOngkir) || 0);
                },

                totalAdj() {
                    const itemAdj = this.rows.reduce((acc, r) => acc + (parseFloat(r.adjustment) || 0), 0);
                    return itemAdj + (parseFloat(this.headerAdj) || 0);
                },
